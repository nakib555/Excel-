            ...
            <div className="flex flex-col gap-6 h-full">
                <GroupBox label="Orientation" className="flex-1 flex flex-col min-h-[360px]">
                    <div className="flex-1 flex flex-col items-center justify-between py-2 gap-8">
                        {/* Clock Visualization */}
                        <div className="relative w-44 h-44 rounded-full border-4 border-slate-100 bg-white shadow-[inset_0_4px_12px_rgba(0,0,0,0.05),0_10px_25px_-5px_rgba(0,0,0,0.05)] flex items-center justify-center group/orient">
                            {/* Inner Circle Mesh */}
                            <div className="absolute inset-2 rounded-full border border-slate-50 bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.02)_0%,transparent_100%)]" />

                            {/* Tick Marks */}
                            {[...Array(12)].map((_, i) => (
                                <div key={i} className="absolute h-full w-[1.5px]" style={{ transform: `rotate(${i * 30}deg)` }}>
                                    <div className={cn(
                            ...
                            {/* Active Pointer */}
                            <div 
                                className="absolute h-[4px] w-[46%] bg-gradient-to-r from-primary-400 to-primary-600 origin-left top-1/2 left-1/2 transition-transform duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) shadow-[0_4px_10px_-2px_rgba(16,185,129,0.3)] rounded-full z-20"
                                style={{ transform: `rotate(${(style.textRotation || 0) * -1}deg)` }}
                            >
                                <div className="absolute -right-3 -top-2.5 w-6 h-6 bg-white border-4 border-primary-600 rounded-full shadow-xl ring-8 ring-primary-500/10 cursor-grab active:cursor-grabbing hover:scale-110 transition-transform" />
                            </div>

                            {/* Axis Point */}
                            <div className="w-4 h-4 bg-white rounded-full z-30 border-4 border-slate-300 shadow-sm" />

                            {/* Legend Tags */}
                            <div className="absolute left-1/2 top-2 -translate-x-1/2 text-[10px] font-black text-slate-300 tracking-tighter">90°</div>
                            <div className="absolute left-1/2 bottom-2 -translate-x-1/2 text-[10px] font-black text-slate-300 tracking-tighter">-90°</div>
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 tracking-tighter">0°</div>
                        </div>
                        ...
                        <div className="w-full flex flex-col gap-4 mt-2">
                            ...
                            <div className="flex gap-2">
                                <button 
                                    onClick={() => { onChange('verticalText', !style.verticalText); if(!style.verticalText) onChange('textRotation', 0); }}
                                    className={cn(
...
const FormatCellsDialog: React.FC<FormatCellsDialogProps> = ({ isOpen, onClose, initialStyle, onApply }) => {
  ...
  const floatingClass = isMobile 
    ? "fixed bottom-0 left-0 right-0 h-[85vh] w-full rounded-t-[40px] shadow-[0_-30px_80px_-10px_rgba(0,0,0,0.3)] z-[2001] bg-white flex flex-col overflow-hidden" 
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-slate-900/40 backdrop-blur-md pointer-events-auto">
        <AnimatePresence>
            {isOpen && (
                <motion.div 
                ...
                >
                    ...
                    <div 
                        ref={dragRef}
                        className={cn(
                            "h-20 flex items-center justify-between px-10 select-none flex-shrink-0 relative",
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 mt-2">
                             ...
                             <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Properties & Styles</span>
                             </div>
                        </div>
                    ...
                    {/* Modern Navigation */}
                    <div className="px-10 py-1 flex-shrink-0">
                            ...
                            {TABS.map(tab => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "flex-1 px-5 py-3 text-[13px] font-black rounded-[18px] transition-all whitespace-nowrap",
                                            active 
                    ...
                    {/* Main Content Area */}
                    <div className="flex-1 bg-white p-10 overflow-y-auto scrollbar-thin">
                    ...
                    <div className="h-28 border-t border-slate-100 bg-slate-50/50 backdrop-blur-xl flex items-center justify-end px-10 gap-5 flex-shrink-0 pb-4">
                        <button 
                            onClick={onClose} 
                            className="px-8 py-3.5 rounded-2xl text-[14px] font-black text-slate-400 hover:text-slate-900 transition-all active:scale-95"
                        >
                            Discard
                        </button>
                        <button 
                            onClick={handleApply} 
                            className="px-14 py-3.5 bg-slate-900 rounded-[22px] text-[14px] font-black text-white hover:bg-slate-800 shadow-2xl shadow-slate-900/20 active:scale-95 transition-all"
                        >
                            Save Changes
                        </button>
                    </div>
...
const BorderTab = ({ style, onChange, isMobile }: { style: CellStyle, onChange: any, isMobile: boolean }) => (
    <div className="flex flex-col gap-6 h-full justify-center items-center text-center px-6">
        ...
        <div>
            <h3 className="text-[19px] font-black text-slate-800 tracking-tight">Style Borders</h3>
            <p className="text-[14px] text-slate-500 mt-2 leading-relaxed font-medium">
