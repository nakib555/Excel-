...
import { 
  FilePlus, FolderOpen, Save, FileDown, Printer, Settings, Share2, LogOut, 
  History, Info, Shield
} from 'lucide-react';
import { RibbonGroup, RibbonButton, TabProps } from '../shared';

const FileTab: React.FC<TabProps> = ({ onExport, onClear }) => {
  return (
    <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
    ...
    >
        <RibbonGroup label="Workbook">
            <div className="flex items-center gap-1 h-full">
                ...
                <RibbonButton 
                    variant="large" 
                    icon={<Save size={20} className="text-slate-700" />} 
                    label="Save" 
                    subLabel="As"
                    onClick={() => {}} 
                />
        ...
        <RibbonGroup label="Export">
            <div className="flex items-center gap-1 h-full">
                <RibbonButton 
                    variant="large" 
                    icon={<FileDown size={20} className="text-green-600" />} 
                    label="Export" 
                    subLabel="CSV" 
                    onClick={onExport} 
                />
